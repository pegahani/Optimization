...
\author[Alizadeh, Osmani, Traversi]{\underline{Pegah Alizadeh}, Aomar Osmani, Emiliano Traversi}
%%\institute[LIPN]{\emph{LIPN - CNRS - Univ. Paris 13 - USPC}}
\institute[LdV, LIPN]{\emph{Pôle Universitaire Léonard de Vinci\\ LIPN, Université Paris13 - Paris}}
...
\begin{frame}
\frametitle{Why MDPs?}
\imp{Markov Decision Processes (MDPs)} are effective models for representing and solving \imp{sequential decision problems}. %(autonomous vehicles, robotics or web service composition).

\centering \includegraphics[scale=0.2]{./images/car-race.pdf} {\tiny{source: internet}}

\begin{itemize}
\item Applications: autonomous vehicles, robotics, web service composition. 
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{Markov Decision Process (MDP) - basic definitions}

   \begin{minipage}{0.4\linewidth}
      %\centering
       \includegraphics[scale=0.22]{./images/mdp.png} {\tiny{source: wikipedia}}     
   \end{minipage}\hfill
\begin{minipage}{0.6\linewidth}  

\begin{block}{Markov Decision Process (MDP)}
%A \textit{Markov Decision Process (MDP)} 
a tuple $M(S, A, P, r, \gamma, \beta)$, where:
\begin{itemize}
\item
 $S$ is a finite set of states.
 \item $A$ is finite set of actions.
\item $P: S \times A \times S \longrightarrow [0,1]$ is a \textit{transition function} 
%where:
...
{\color{blue}{Goal:}} maximize the expected sum of rewards.  \includegraphics[scale=0.1]{./images/car-race.pdf}
...
\begin{block}{}
{\color{blue}Value function:} $V^{\tilde{\pi}}(s) = \mathbb{E}(\sum_{t=0}^{\infty} \gamma^{t} r(s_t, \tilde{\pi}(s_t))$.\\
\vspace{0.3cm}
{\color{blue}Visitation frequency function:}\\ %$f^{\tilde{\pi}}$
...
$$
\vspace{0.3cm}
{\color{blue} Relation :} $\beta \cdot V^{\tilde{\pi}} = r \cdot f^{\tilde{\pi}}$
\end{block}
\pause

\begin{itemize}
\item The policy $\tilde{\pi}(s,a)$ is computable from $f^{\tilde{\pi}}$:
\begin{align*}
...
\item For \textbf{simplicity of our experiments} we work with a box as uncertainty set.
\end{itemize}
\end{frame}
...
\item As a game, the adversary tries to find a reward value $r$ in order to maximise our loss for the given $f^{\pi}$.  
\end{itemize}
    }
        \only<4>{
...
The minimax regret of feasible reward set $\mathcal{R}$ is:
\vspace{-0.2cm}
...
%%    \only<2,3,4>{
%\begin{itemize}
%\item $f$ must be a \textit{valid visitation frequency function}:\\
%$f^{\tilde{\pi}}(s, a) = \sum_{s' \in S} \beta(s') \sum_{t=0}^{\infty} \gamma^{t-1}P(S_t = s', A_t = a | S_1 = s)$\\
%\item[] 
...
%%        \only<5,6,7>{
%\begin{itemize}
%\item $GEN=$ all combinations of \textit{rewards $r$} and corresponding \textit{optimal adversary policies $g_r$}.\\
%\item[] 
...
\textbf{Goal}: \imp{provide an optimal deterministic policy} for an IRMDP.

\begin{columns}
\begin{column}{0.4\textwidth}
A branch by selecting a couple \imp{$(s,a)$} and disjuncting on two nodes:
\begin{itemize}
...
\begin{frame}
\frametitle{\textit{Benders Decomposition} + \textit{Branch-and-Bound}}
\textbf{Cut-and-branch}, an improved version.
...
%\pause
\item[] $\Rightarrow$ \textbf{we explore more nodes but faster.}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{\textit{Determinised} policy: an obvious approach}
\begin{itemize}
\item obtain a deterministic policy from a stochastic policy.
...
for the optimal stochastic policy $\tilde{f}$, the ``rounding'' determinised policy $\hat{\pi}$:
\begin{itemize}
...
\textbf{ New }set of instances.
\item Diamond MDPs (\texttt{Diamond}).\\
 From the \textbf{literature} and \textbf{new}.
\end{itemize}
\begin{block}{Value Ratio ($VR$)}
$MR(f^{\hat{\pi}}, \mathcal{R})$ be the maximum regret of the determinised policy $MR(f^{\pi^*}, \mathcal{R})$  be the maximum regret of the optimal deterministic policy.\\

\textit{Value Ratio} of MDP: $VR = \dfrac{MR(f^{\hat{\pi}}, \mathcal{R})}{MR(f^{\pi^*}, \mathcal{R})}\;$.
\end{block}
\end{frame}
...
\input{tab_random_diamond_slim.tex}
	\centering \includegraphics[scale=0.2]{./images/diamond.png}  
    }
\end{overlayarea}
\end{frame}
...
\begin{itemize}
\item First time algorithm for finding an \textbf{optimal deterministic policy} using \textbf{minimax regret on IRMDPs}.
\item Showed \textbf{theoretically} and \textbf{experimentally} why finding the deterministic policy is important.
\item Proposed \textbf{faster methods} than our original branch and bound on minimax regret calculation.
