...
\PA{The transition function has the following properties: from any state $s$ the number of reachable states is equal to $n$, uniformly selected over the set of states. The number of actions depends on the value of $n$. First, we define $n$ actions, where each of them reaches only a single state among the allowed ones. Secondly, each of the remaining actions reaches two states.} Therefore, we have a total of $n+\frac{n(n-1)}{2}$ actions. The initial state distribution $\beta$ is uniform and we choose a discount factor $\gamma = 0.95$.
...
\PA{In this family of problems, the reward of a few states suffices to generate a lot of uncertainties about the optimal policy.} %This IRMDP is an interesting set of instances to test our proposed algorithm. 
...
%
With \texttt{Random-unlim} MDPs, each action leads to  $\lceil \log_2(n) \rceil$ states (\PA{while for the other two classes of MDPs each action leads to two states}). Therefore, for \texttt{Random-unlim} MDPs, even a deterministic policy can visit a significant amount of states.
%
...
%
Therefore, choosing to determinise may turn out to be an unsafe choice, specially considering that it is not possible to check how far a determinisied policy is from the optimal.  
...
\begin{figure}[]
	...
	\end{center}
	\caption{Impact of cut-and-branch, \texttt{Random-unlim} MDPs. \PA{add axis names to the graph}}
...
\begin{figure}[]
	...
	\end{center}
	\caption{Impact of cut-and-branch, \texttt{Random-lim} MDPs. \PA{add axis names to the graph}}
