...
It is also interesting to notice that the maximum values are quite high for almost all the combinations of parameters considered. This implies that chosing to determinise my reveal as a unsafe choice, specially considering that there is no way to check how far a determinisied policy is from the optimal.  
