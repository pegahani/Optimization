...
The approach computes the deterministic policy by selecting the action with the highest probability for each state. Despite being pretty simple, this approach represents a plausible behavior of a user that want to derive a deterministic policy starting from a stochastic one. \ET{From now on, we will name this rounding deterministic policy as ``deteminised policy''.}
%
