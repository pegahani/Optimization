...
\author{Paper 17}  % put your paper number here!
...
%to get the expected results the policy needs to be repeated for a great number of times and/or for a very long time with independent and identically distributed draws of actions. In contrast, 
...
Similar to several previous works in the literature \cite{Ahmed2017,alizadeh2015,benavent2018,Regan2009,Weng2013}, we assume that the set of possible rewards is modelled as a polytope $\mathcal{R} = \{r: Cr \leq d \}$. The only real restriction we make concerning the uncertainty set is that we want it to be convex because it is well known that any convex set can be approximated with a polytope. For simplicity, in our experiments we work with a box as uncertainty set. However, our method can be extended to any generic polytope. 
% More precisely, we suppose that, each $r(s,a) \in r$ is restricted in an interval. 
...
The algorithm used to achieve this goal is a branch-and-bound framework  that uses the Benders decomposition procedure described in the previous section as bounding procedure. The branch-and-bound algorithm is one of the most widely and effective algorithms used to solve combinatorial problems since decades (see \cite{bertsimas2005optimization}, Section $11$ for an exhaustive explanation of a generic version of the branch-and-bound algorithm).
 %Any equivalent algorithm can be used to illustrate our contribution. 
