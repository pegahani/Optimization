...
Similar to several previous works in the literature \cite{Ahmed2017,alizadeh2015,benavent2018,Regan2009,Weng2013}, we assume that the set of possible rewards is modelled as a polytope $\mathcal{R} = \{r: Cr \leq d \}$. \ET{The only real restriction we make concerning the uncertainty set is that we want it to be convex bacause it is well known that any convex set can be approximated with a polytope. For simplicity, in our experiments we work with a box as uncertainty set. However, our method can be extended to any generic polytope. }
...
The algorithm used to achieve this goal is a branch-and-bound framework  that uses the Benders decomposition procedure described in the previous section as bounding procedure.
\ET{The branch-and-bound algorithm one of the is the most widely and effective algorithms used to solve combinatorial problems since decates (see \cite{bertsimas2005optimization}, Section $11$ for an exhaustive explanation of a generic version of the branch-and-bound algorithm).}
