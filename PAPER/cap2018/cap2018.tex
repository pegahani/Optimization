...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ET}[1]{{\textcolor{black}{#1}}}
...
\title{Stochastic vs deterministic policies for Markov Decision Processes}
...
\begin{abstract}
In several real world applications of sequential decision making under uncertainty a stochastic policy is not easily interpretable for the system users. This might be due to the nature of the problem or to the system requirements. In these contexts, it is more convenient (inevitable) to provide a deterministic policy to the user. 
In this work we compare the behaviour of deterministic and stochastic policies for minimizing the maximum regret in Markov Decision Processes (MDPs). 
%In this paper, we propose an approach for computing a deterministic policy for a Markov Decision Process with Imprecise Rewards in reasonable computational overhead in comparison to the required time for computing the optimal stochastic policy. To better motivate the use of an exact procedure for finding a deterministic policy, we show some cases where the intuitive idea of using a deterministic policy obtained after ``determinising" (rounding) the optimal stochastic policy leads to a deterministic policy different from the optimal.
\end{abstract}


\medskip

\noindent\textbf{Mots-clef}: Markov Decision Processes, Minimax Regret, Unknown Rewards, Deterministic Policy, Stochastic Policy

%\textcolor{red}{ FROM EMILIANO: I removed the following things from the text:\\
%1- the improved version of the branch-and-bound (what we called cut-and-branch in the other paper).\\ 
%2- the counterexample.\\
%I think I removed from the text any reference to one of the two points. If you find some, feel free to delete it.
%}
...
Mannor et al. \cite{Mannor2007} demonstrate that the strategy found via an optimisation process under the MDPs with numerical parameters, sometimes can be much worse than the anticipated policy. This can happen for multiple reasons:
%\begin{itemize}
%\item 
...
We show that the use of an intuitive rounding technique to obtain a feasible deterministic policy based on the optimal stochastic solution can lead to a policy significantly different from the optimal. 
...
%we use an 
MDPs with \textit{imprecise reward values} (IRMDP). An IRMDP \cite{Regan2009} is a tuple $M(S, A, P, r, \gamma, \beta)$ where $S, A, P, \gamma$ and $\beta$ are defined as in the previous section, while $r$ is a set of possible reward functions on $S \times A$. $r$ models the uncertainty on real reward values. 
...
In this paper, we use the approach presented by Regan and Boutilier \cite{Regan2009} based on \textit{Benders Decomposition} \cite{Benders1962}.
...
%%%%%%
\begin{alignat}{3}
&\text{min}_{\delta, f} && \delta & \\
&\text{s.t.:}&\quad& r\cdot g - r \cdot f \leq \delta \quad \forall \langle g_r, r \rangle \in \text{GEN}\label{delta_cut}\\
...
\texttt{Slave Program}
\begin{alignat}{3}
&\text{max}_{Q, V, I, r} && \beta \cdot V - r \cdot f \\
&\text{s.t.:} &\quad& Q_a = r_a + \gamma P_aV &\quad \forall a \in A\\
...
The intuition behind this constraint is related to the dual linear program of the Bellman Equation (see for example \cite{Sutton1998}, Chapter $4$ or \cite{Puterman1994}, Section $6.9$). 
...
%, in other words, it finds a $\bar{r}$ and $\bar{g}$ such that $\bar{r} \cdot \bar{g}  - \bar{r}  \cdot f^* > \delta^*$. If such a $(\bar{r},\bar{g})$ is found, it is added to GEN and the master problem is solved again. 
...
\section{Exact and heuristic approaches}\label{sec:bb} 

\paragraph{\textbf{A branch-and-bound scheme to find the optimal deterministic solution.}}
...
\paragraph{\textbf{A ``rounding'' heuristic.}}
%
...
In this section we introduce the intuitive concept of rounding heuristic, a way to obtain a feasible deterministic policy starting from a stochastic optimal policy. Such heuristic will be compared with the optimal algorithm proposed in the previous Section. \\
